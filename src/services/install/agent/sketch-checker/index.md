...
- **状态文件只读**：禁止直接新建、修改或删除 `.sketch-cache/` 目录下的状态文件
...
| sketch-draw-check                                | 告知用户当前画板已完成，建议在处理下一个画板前手动执行 `/compact` 清理上下文，之后：<br>委托subagent：sketch-analyzer 调用skill：sketch-preview                                      |
