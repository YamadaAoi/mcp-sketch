...
- 若输出中标明 `NEXT_STEP_RECOMMENDATION`，动态调整 todo 列表
  - **积极采纳 subagent 的专业建议**，除非建议不利于工作流或与用户意图冲突
  - 若建议中包含 `/compact`，按以下流程处理：
    - 检查当前状态文件是否完整记录，**若未记录则先记录**
    - 调用命令 `/compact 保留当前设计稿路径、页面名、画板名、当前进度、下一步计划和用户反馈（如有）`
    - compact 结束后，读取progress.json状态文件确认状态
    - 根据下步计划继续推进
