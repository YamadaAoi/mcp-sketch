...
审核父组件的布局是否符合要求：子组件容器包裹、响应式布局、lint/typecheck
...
1. 组件文件是否存在
2. 每个直接子组件是否有 `div` 容器包裹，类名 `{sub-component-name}-wrap`
3. 每个子组件是否已正确 import
4. 容器样式是否为响应式布局（灵活运用 `%`、`flex`、`calc`）
5. **如果出现 `position: absolute`、固定 `px` 布局或 `@media` 媒体查询**，标记为问题
...
布局审核失败：
- 组件路径：{component_path}
- 问题类型：{容器缺失 | 非响应式布局 | lint错误 | typecheck错误}
- 问题描述：{具体问题}
- 修复建议：{建议如何修复}
LAYOUT_CHECK_FAILED
```
