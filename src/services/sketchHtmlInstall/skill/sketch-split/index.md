...
| `--ap`     | 切图存放路径，从 `proj-init.md` 约定的静态资源目录。切图会自动压缩为 webp 格式。split 阶段一般不需要，若判断组件边界有歧义时可传入以读取切图辅助分析 |
...
- **情况 1：一个图层精确对应一个视觉区域**
  - 判断条件：图层的 rect 与视觉区域在上下左右 4 个方向上的平均误差 < 10%
  - 计算方式：上下方向误差基于图层高度，左右方向误差基于图层宽度，取 4 个方向误差的平均值
