...
- 此技能通常由 `skill: sketch-split` 触发，用于处理特定矩形区域（rect）的代码生成。
- 调用前应具备：
...
**为什么需要 exclude_rects？**
当绘制父组件时，若其子组件区域也在父组件的 `rect` 内，`analyze` 会返回子组件区域内的所有图层。由于子组件后续由 `skill: sketch-draw` 独立绘制，父组件不应包含这些内容 — 它只需要"背景层"。将子组件的 rects 作为 `exclude_rects` 传入，即可从分析结果中排除它们，避免重复渲染。
...
当被 `skill: sketch-workflow` 批量调用时：
