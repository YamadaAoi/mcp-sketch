...
在生成代码前，**必须**从 `.md` 描述文档中读取 `type` 字段，根据组件类型采用不同策略：

| type            | 组件角色               | 生成策略                                   |
| --------------- | ---------------------- | ------------------------------------------ |
| `page`          | 页面入口组件（父组件） | 绘制背景、布局容器，并 import + 调用子组件 |
| `common`        | 公共组件               | 按常规方式绘制                             |
| `page-specific` | 页面特有组件           | 按常规方式绘制                             |

**父组件（type: page）的特殊处理规则**：
...
- [ ] **父组件（type: page）未 import 子组件**
- [ ] **父组件（type: page）未在模板中调用子组件**
- [ ] **父组件（type: page）绘制了子组件区域内的具体内容**
